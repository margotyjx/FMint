...
def write_into_dict(result_dict, task, runner, equation, all_caption, all_data, label, test_demo_num_list, test_caption_id_list, split_data):
	...
	if -1 in test_caption_id_list_in_use:
		...
		print('batch of sample takes: ', time.time() - start)
			# if FLAGS.backend == 'jax':
...
# 加一个对error correct后的plot
	...
	print("TOTAL TIME DURING INFERENCE STAGE FOR ALL TESTING FAMILIES OVER 100 CASES: ", total_time)
...
# 			RMSE[key] = []
# 		if flag_2D:
# 			rel_err[key].append(np.mean(np.abs(preds_demo_1[key] - ground_truths[key])/np.abs(ground_truths[key])))
# 			rel_err[key].append(np.mean(np.abs(preds_demo_2[key] - ground_truths[key])/np.abs(ground_truths[key])))
# 			rel_err[key].append(np.mean(np.abs(preds_demo_3[key] - ground_truths[key])/np.abs(ground_truths[key])))
# 			rel_err[key].append(np.mean(np.abs(preds_demo_4[key] - ground_truths[key])/np.abs(ground_truths[key])))
# 			rel_err[key].append(np.mean(np.abs(preds_demo_5[key] - ground_truths[key])/np.abs(ground_truths[key])))

# 			RMSE[key].append(np.sqrt(np.mean((preds_demo_1[key] - ground_truths[key])**2)))
# 			RMSE[key].append(np.sqrt(np.mean((preds_demo_2[key] - ground_truths[key])**2)))
# 			RMSE[key].append(np.sqrt(np.mean((preds_demo_3[key] - ground_truths[key])**2)))
# 			RMSE[key].append(np.sqrt(np.mean((preds_demo_4[key] - ground_truths[key])**2)))
# 			RMSE[key].append(np.sqrt(np.mean((preds_demo_5[key] - ground_truths[key])**2)))

# 			if FLAGS.correction:
# 				err_coarse[key].append(np.sqrt(np.mean((coarse_u[key][0] - ground_truths[key])**2)))
# 				err_coarse[key].append(np.mean(np.abs(coarse_u[key][0] - ground_truths[key])/np.abs(ground_truths[key])))
# 		else:
# 			rel_err[key].append(np.mean(np.abs(preds_demo_1[key][:,:,0] - ground_truths[key][:,:,0])/np.abs(ground_truths[key][:,:,0])))
# 			rel_err[key].append(np.mean(np.abs(preds_demo_2[key][:,:,0] - ground_truths[key][:,:,0])/np.abs(ground_truths[key][:,:,0])))
# 			rel_err[key].append(np.mean(np.abs(preds_demo_3[key][:,:,0] - ground_truths[key][:,:,0])/np.abs(ground_truths[key][:,:,0])))
# 			rel_err[key].append(np.mean(np.abs(preds_demo_4[key][:,:,0] - ground_truths[key][:,:,0])/np.abs(ground_truths[key][:,:,0])))
# 			rel_err[key].append(np.mean(np.abs(preds_demo_5[key][:,:,0] - ground_truths[key][:,:,0])/np.abs(ground_truths[key][:,:,0])))

# 			RMSE[key].append(np.sqrt(np.mean((preds_demo_1[key][:,:,0] - ground_truths[key][:,:,0])**2)))
# 			RMSE[key].append(np.sqrt(np.mean((preds_demo_2[key][:,:,0] - ground_truths[key][:,:,0])**2)))
# 			RMSE[key].append(np.sqrt(np.mean((preds_demo_3[key][:,:,0] - ground_truths[key][:,:,0])**2)))
# 			RMSE[key].append(np.sqrt(np.mean((preds_demo_4[key][:,:,0] - ground_truths[key][:,:,0])**2)))
# 			RMSE[key].append(np.sqrt(np.mean((preds_demo_5[key][:,:,0] - ground_truths[key][:,:,0])**2)))

# 			if FLAGS.correction:
# 				err_coarse[key].append(np.sqrt(np.mean((coarse_u[key][0][:,:,0] - ground_truths[key][:,:,0])**2)))
# 				err_coarse[key].append(np.mean(np.abs(coarse_u[key][0][:,:,0] - ground_truths[key][:,:,0])/np.abs(ground_truths[key][:,:,0])))
		
# 	print('RMSE: ', RMSE)
# 	print('relative errors: ', rel_err)
# 	if FLAGS.correction:
# 		print('relative error of coarse u', err_coarse)
...
# 		elif 'gravity' in eqn:
# 			l = 'Pendulum gravity'
# 		elif 'drivendamped_pendulum' in eqn:
# 			l = 'Driven damped pendulum'
# 		axs.plot(FLAGS.test_demo_num_list, rmse, label = l)
# 		# axs.fill_between(FLAGS.test_demo_num_list, np.array(rmse) - np.array(std[eqn]), np.array(rmse) + np.array(std[eqn]), alpha = 0.2)
# 	axs.set_xlabel("demo number", fontsize = 20)
# 	axs.set_ylabel('RMSE', fontsize = 20)
# 	axs.set_title("RMSE vs demo number")
# 	axs.set_xticks(FLAGS.test_demo_num_list)
# 	axs.legend(fontsize = "20")
	
# 	fig.savefig(FLAGS.analysis_dir + '/rmse_demoNum.pdf')

# # plot average relative error
# 	plt.close('all')
# 	plt.rcParams['font.size'] = 20
# 	fig, axs = plt.subplots(1, 1, figsize=(12, 8))
# 	for eqn, error in rel_err.items():
# 		if "expo" in eqn:
# 			l = "Exponential decay"
# 		elif "cooling" in eqn:
# 			l = "Newton's law of cooling"
...
# 		elif 'drivendamped_pendulum' in eqn:
# 			l = 'Driven damped pendulum'
# 		axs.plot(FLAGS.test_demo_num_list, error, label = l)
# 		# axs.fill_between(FLAGS.test_demo_num_list, np.array(rmse) - np.array(std[eqn]), np.array(rmse) + np.array(std[eqn]), alpha = 0.2)
# 	axs.set_xlabel("demo number", fontsize = 20)
# 	axs.set_ylabel('relative error', fontsize = 20)
# 	axs.set_title("Relative error vs demo number")
# 	axs.set_xticks(FLAGS.test_demo_num_list)
# 	axs.legend(fontsize = "20")
	
# 	fig.savefig(FLAGS.analysis_dir + '/relative_error_demoNum.pdf')
...
if __name__ == '__main__':
	...
	flags.DEFINE_boolean('correction', False, 'nv or icon')
	...
	flags.DEFINE_list('test_data_dirs', '/export/jyuan98/learn2correct/data_preparation/icon_test', 'directories of testing data')
	...
	flags.DEFINE_list('test_demo_num_list', [1], 'demo number list for testing')
	...
	flags.DEFINE_string('analysis_dir', '/export/jyuan98/learn2correct/analysis/', 'write file to dir')
	...
	flags.DEFINE_integer('restore_step', 500000, 'restore step')
